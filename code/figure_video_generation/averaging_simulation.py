...
def graph_product(G: Graph, H: Graph) -> Graph:
    """
    Compute the product of two graphs G and H with implicit self-loops.

    The product graph has the same node set as G and H.
    There is an edge (a, c) in the product if there exists a node b such that:
    - (a, b) is an edge in G or a == b (implicit self-loop), AND
    - (b, c) is an edge in H or b == c (implicit self-loop)

    Self-loops are used in the computation but not stored in the final graph.

    Args:
        G: First directed graph
        H: Second directed graph

    Returns:
        A new Graph representing the product of G and H (without self-loops)

    Raises:
        ValueError: If G and H have different number of nodes
    """
    if G.num_nodes != H.num_nodes:
        raise ValueError(f"G and H must have the same number of nodes. Got {G.num_nodes} and {H.num_nodes}")

    num_nodes = G.num_nodes
    product_edges: set[tuple[int, int]] = set()

    # For each pair of nodes (a, c), check if there exists b such that
    # (a,b) in G or a==b (self-loop) AND (b,c) in H or b==c (self-loop)
    for a in range(num_nodes):
        for c in range(num_nodes):
            # Get outgoing neighbors of a in G, plus a itself (self-loop)
            neighbors_a_in_G = set(G.get_neighbors(a)) | {a}
            # Get incoming neighbors of c in H (nodes b such that (b,c) in H), plus c itself (self-loop)
            neighbors_c_in_H = set(H.get_in_neighbors(c)) | {c}

            # Check if there's a common node b
            if neighbors_a_in_G & neighbors_c_in_H:
                # Add edge (a, c) only if it's not a self-loop
                if a != c:
                    product_edges.add((a, c))

    return Graph(list(product_edges), num_nodes)


def make_product_sequence(graphs: list[Graph]) -> list[Graph]:
    """
    Create a sequence of product graphs from pairs of successive graphs.

    Takes graphs (G_0, G_1, G_2, G_3, ...) and returns the product sequence
    where each element is the product of pairs: (G_0 ⊙ G_1), (G_2 ⊙ G_3), ...

    Args:
        graphs: A list of graphs

    Returns:
        A list of product graphs, with length = len(graphs) // 2
    """
    product_graphs = []
    for i in range(0, len(graphs) - 1, 2):
        product = graph_product(graphs[i], graphs[i + 1])
        product_graphs.append(product)
    return product_graphs


def broadcasting_graph(n: int, k: int, seed: int = 42) -> tuple[Graph, set[int]]:
    """
    Create a broadcasting graph with n nodes and k broadcasters.

    A broadcasting graph has a set of k randomly chosen nodes (broadcasters).
    Each node in the graph (including broadcasters) has an incoming edge from
    one randomly chosen broadcaster. Self-loops are implicit and not stored.

    Args:
        n: Number of nodes
        k: Number of broadcaster nodes
        seed: Random seed for reproducibility

    Returns:
        A tuple (graph, broadcasters) where:
        - graph: Graph where each node has exactly one incoming edge from a broadcaster
                 (self-loops are implicit but not stored)
        - broadcasters: Set of k broadcaster node indices
    ...
    # Randomly select k broadcasters from n nodes
    broadcasters = np.random.choice(n, size=k, replace=False)
    broadcasters_set = set(int(b) for b in broadcasters)

    # For each node, create an edge from a random broadcaster
    edges = []
    for node in range(n):
        # Pick a random broadcaster to send to this node
        broadcaster = np.random.choice(broadcasters)
        edges.append((int(broadcaster), node))

    # Remove self-loops (keep implicit)
    edges = [(u, v) for u, v in edges if u != v]

    return Graph(edges, n), broadcasters_set
...
def generate_random_graph_sequence(graphs: list[Graph], length: int, seed: int = 42) -> list[Graph]:
    """
    Generate a random sequence of graphs from a given set.
    
    Args:
        graphs: List of available graphs to choose from
        length: Length of the sequence to generate
        seed: Random seed for reproducibility (default: 42)
        
    Returns:
        List of graphs in random order with the specified length
    """
    if not graphs:
        raise ValueError("Graphs list cannot be empty")
    
    np.random.seed(seed)
    return [graphs[i % len(graphs)] for i in np.random.randint(0, len(graphs), length)]
...
def pull_graph(components: list[list[int]], pull_edge: tuple[int, int]) -> Graph:
    """
    Create a graph with specified strongly connected components and a pull edge.

    Each component is internally strongly connected, and the pull_edge connects
    one component to another.

    Args:
        components: List of components, each as a list of node indices
                   e.g., [[0], [1, 2, 4]]
        pull_edge: Tuple (from_node, to_node) specifying the edge connecting components
                   e.g., (0, 1) means edge from node 0 to node 1

    Returns:
        Graph with the specified structure
    """
    # Determine total number of nodes
    num_nodes = max(max(comp) for comp in components) + 1
    ...
    for component in components:
        if len(component) > 1:
            # Create a cycle within the component to make it strongly connected
            for i in range(len(component)):
                from_node = component[i]
                to_node = component[(i + 1) % len(component)]
                edges.append((from_node, to_node))

    # Add the pull edge
    edges.append(pull_edge)
...
def run_broadcasting(n: int, k: int, num_graphs: int = 20, seed: int = 42, d: int = 3) -> tuple[list[list[NDArray[np.float64]]], list[set[int]], list[list[NDArray[np.float64]]]]:
    """
    Run the averaging algorithm on a sequence of broadcasting graphs.

    Generates num_graphs broadcasting graphs with n nodes and k broadcasters,
    then runs the averaging algorithm on this sequence. Computes normalized
    difference vectors between broadcaster node values at each time step.

    Args:
        n: Number of nodes in each graph
        k: Number of broadcaster nodes per graph
        num_graphs: Number of broadcasting graphs to generate (default 20)
        seed: Random seed for reproducibility (default 42)
        d: Dimension of node values (default 3)

    Returns:
        A tuple (x, broadcasters_list, vectors_over_time) where:
        - x: List of states x[t][i] representing node values at each time step
        - broadcasters_list: List of broadcaster sets for each graph
        - vectors_over_time: List of lists of normalized vectors. vectors_over_time[t][i]
                             is the normalized difference vector at time t for graph i
    """
    # Generate initial values
    x0 = generate_initial_values(n, d, seed=seed)

    # Generate sequence of broadcasting graphs and track broadcasters
    graph_sequence = []
    broadcasters_list = []

    for i in range(num_graphs):
        graph, broadcasters = broadcasting_graph(n, k, seed=seed + i)
        graph_sequence.append(graph)
        broadcasters_list.append(broadcasters)

    # Run the averaging algorithm
    x = run_averaging_algorithm(graph_sequence, x0, num_graphs)

    # Compute normalized difference vectors between broadcasters at each time step
    vectors_over_time = []
    for t in range(len(x)):
        vectors_at_t = []
        for broadcasters in broadcasters_list:
            b1, b2 = sorted(broadcasters)
            # Difference vector between broadcaster values
            vector = x[t][b2] - x[t][b1]
            # Normalize to unit length
            length = np.linalg.norm(vector)
            normalized_vector = vector / length if length > 0 else vector
            vectors_at_t.append(normalized_vector)
        vectors_over_time.append(vectors_at_t)

    return x, broadcasters_list, vectors_over_time


def plot_broadcaster_vectors_3d(vectors_over_time: list[list[NDArray[np.float64]]]) -> None:
    """
    Plot all broadcaster difference vectors in a single 3D figure.

    Plots all vectors across all time steps on a single unit sphere.
    Overlapping vectors create darker regions due to transparency (alpha blending).
    ...
    ax.plot_surface(x_sphere, y_sphere, z_sphere, alpha=0.05, color='gray')

    # Plot all vectors as lines through origin (since x and -x are equivalent)
    # This shows projection axes rather than directions
    for t, vectors_at_t in enumerate(vectors_over_time):
        for v_vec in vectors_at_t:
            ...
            if v_length < 1e-10:
                continue
            # Normalize to unit length for line drawing
            v_normalized = v_vec / v_length
            # Draw line from -v to +v through origin
            ax.plot([-v_normalized[0], v_normalized[0]],
                   [-v_normalized[1], v_normalized[1]],
                   [-v_normalized[2], v_normalized[2]],
    ...
    plt.tight_layout()
    os.makedirs("results", exist_ok=True)
    output_path = os.path.join("results", "broadcaster_vectors.pdf")
    ...
    print(f"Broadcaster vectors visualization saved: {output_path}")


def animate_broadcaster_vectors(vectors_over_time: list[list[NDArray[np.float64]]], skip: int = 5) -> None:
    """
    Create an animation of broadcaster projection lines evolving over time.

    Args:
        vectors_over_time: List of lists where vectors_over_time[t] contains vectors at time t
        skip: Number of time steps to skip between frames (default 5)
    """
    ...
    # Plot unit sphere (wireframe) once
    u = np.linspace(0, 2 * np.pi, 20)
    v = np.linspace(0, np.pi, 10)
    x_sphere = np.outer(np.cos(u), np.sin(v))
    y_sphere = np.outer(np.sin(u), np.sin(v))
    ...
    def update(frame):
        ...
        t = frame * skip
        if t >= len(vectors_over_time):
            t = len(vectors_over_time) - 1

        # Plot all vectors up to time t (accumulation)
        for s in range(0, t + 1):
            vectors_at_s = vectors_over_time[s]
            for v_vec in vectors_at_s:
                v_length = np.linalg.norm(v_vec)
                if v_length < 1e-10:
                    continue
                v_normalized = v_vec / v_length
    ...
    num_frames = len(vectors_over_time) // skip + 1
    anim = FuncAnimation(fig, update, frames=num_frames, interval=100, repeat=True)

    os.makedirs("results", exist_ok=True)
    output_path = os.path.join("results", "broadcaster_vectors.mp4")

    writer = FFMpegWriter(fps=10, metadata=dict(artist=''), bitrate=1800)
