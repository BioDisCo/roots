...
vector $x_i(t) \in \IR^d$ in every round $t\in\IN$ such that in every execution holds:
\begin{itemize}
...
The $d$-dimensional hyperplane consensus problem is unsolvable in networks
...
But then~$G$ contains a set~$M$ of $d+1$ processes in which no process is
reachable from any other in~$G$.
Without loss of generality, by permuting the process namees, assume that $M =
\{1,\dots,d+1\}$.
For every $i\in M$, let~$A_i$ be the set of processes that reach~$i$ in~$G$.
By choice of~$M$, the sets~$A_i$ are pairwise disjoint and $i\in S_i$.
Then, for every $j\in A_i$, choose the initial vectors $x_j(0) = e_i$ if $1\leq i \leq d$ where~$e_i$
...
of~$A_i$, no cluster point of the sequence $\big(x_i(t)\big)_t$ can be
different from~$x_i(0)$.
But the initial values, and thus the cluster points, of processes in~$M$ do not
belong to a common hyperplane in~$\IR^d$.
This is contradiction to Hyperplane Agreement.
\end{proof}
