...
\begin{abstract}
We introduce the problem of asymptotic hyperplane consensus, which requires processes' outputs to converge onto a common hyperplane while remaining inside the convex hull of initial vectors.
...
We show that a large class of algorithms used for asymptotic consensus gracefully degrade to solve asymptotic hyperplane consensus in weaker networks models.
\end{abstract}


\section{Introduction}

We show that the problem of asymptotic consensus to hyperplanes of dimension $d$ in network models where
...
We call a set $M\subseteq [n]$ a {\em root set\/} of communication graph $G = ([n],E)$
if for every $i\in[n]$ there exists some $k\in M$ such that $(k,i)\in E$.
