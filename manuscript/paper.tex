...
links, akin to message adversaries~\cite{TODO} or the Heard-Of
model~\cite{TODO}.
That is, every process $i\in[n]$ sends a message in every round $t\in\IN_0$,
...
classical asynchronous~\cite{TODO} and even Byzantine~\cite{TODO} models.
...
$\ell \geq n^{k+1}/\binom{n}{k}\leq n^{k+1}/n^k = n$ communication graphs in which~$M$ is a root set.
...
Since $\ell \geq n-k$, the union of the~$S_i(\ell)$ covers~$[n]$.
